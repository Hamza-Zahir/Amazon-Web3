      ...
      <div class="text p-2">
        ...
        <small class="gift">
          ...
          <span v-if="item.isGift">
            <input
              ...
              @change="
                (e) => {
                  if (e.target.checked !== item.isGift) {
        ...
        <div class="d-flex align-items-center">
              ...
              @change="
                (e) => {
                  if (e.target.value !== item.Quantity) {
                    ChangeQuantity(item.itemName, e.target.value, e.isGift);
                  }
                }
              "
            >
          ...
          <span
            ...
            @click="RemoveItemFromCart(item.itemName, item.isGift)"
            >Delet</span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async RemoveItemFromCart(_itemName, _isGeft) {
      await plugins
        .removeItemFromCart(this.ChainId, this.CurrentAccount, _itemName, _isGeft)
        .then(async () => {
    ...
    async ChangeQuantity(_itemName, _Quantity, _isGeft) {
      await plugins
        .changeQuantity(this.ChainId, this.CurrentAccount, _itemName, _Quantity, _isGeft)
        .then(async () => {
          await this.getItemsOfUser();
        });
    },
