...
contract("Amazon", (accounts) => {
  ...
  it("remove Items from Stoke from account other than the Staff ", async () => {
    await contractInstance
      .removeItemsFromStoke("item1", { from: accounts[2] })
      .then(assert.fail)
      .catch(function (error) {
        assert(
          error.message.indexOf(
  ...
  it("remove Item From Stoke as Staff", async () => {
    ...
    await contractInstance.removeItemsFromStoke(["item1"], {
      from: accounts[0],
    });
    priceOfItem1 = await contractInstance.itemsInStoke("item1");
    assert.equal(priceOfItem1, 0);
  ...
  it("add Item To Cart with item not available in the stoke ", async () => {
    ...
    assert.equal(userCart[0].isGift, true);
  });
  it("add Item To Cart with item available in the stoke ", async () => {
    ...
    assert.equal(userCart[1].isGift, true);
  });
  it("add Item  To Cart and i already have it", async () => {
    await contractInstance.addItemsToCart("item1", 1, true, {
      from: accounts[3],
    });
    const userCart = await contractInstance.getCartItemsOfOwner(accounts[3]);
    assert.equal(userCart[1].itemName, "item1");
    assert.equal(userCart[1].Quantity, 3);
    assert.equal(userCart[1].inStoke, true);
    assert.equal(userCart[1].isGift, true);
  });

  it("change Quantity and set Gift Item", async () => {
    await contractInstance.changeQuantity("item2", 5, true, {
      from: accounts[3],
    });

    await contractInstance.setGift("item1", false, {
      from: accounts[3],
    });
    const userCart = await contractInstance.getCartItemsOfOwner(accounts[3]);
    ...
    assert.equal(userCart[0].isGift, true);
    assert.equal(userCart[1].Quantity, 3);
    assert.equal(userCart[1].isGift, false);
  });
  it("add item as gift then change it to not Gift", async () => {
    await contractInstance.addItemsToCart("item1", 6, true, {
      from: accounts[3],
    });
    const userCart = await contractInstance.getCartItemsOfOwner(accounts[3]);

    assert.equal(userCart[2].Quantity, 6);
    assert.equal(userCart[2].isGift, true);
    assert.equal(userCart[2].itemName, "item1");

    await contractInstance.setGift("item1", false, {
      from: accounts[3],
    });
    const nueUserCart = await contractInstance.getCartItemsOfOwner(accounts[3]);

    assert.equal(nueUserCart[1].Quantity, 9);
    assert.equal(nueUserCart[1].isGift, false);
    assert.equal(nueUserCart[2].Quantity, 0);
    assert.equal(nueUserCart[2].itemName, "");
    assert.equal(nueUserCart[2].isGift, "");
  });

  it("remove Item from Cart", async () => {
    // remove all items
    await contractInstance.removeItemFromCart("item2", true, {
      from: accounts[3],
    });
    await contractInstance.removeItemFromCart("item1", false, {
      from: accounts[3],
    });
    const userCart = await contractInstance.getCartItemsOfOwner(accounts[3]);
  ...
  it("buy Item", async () => {
    ...
    assert.equal(UsersOrders[0].isGift, true);
  });
  it("buy Items", async () => {
    ...
    [true, false, false].map((e, i) => {
      ...
      assert.equal(UsersOrders[i].isGift, e);
      if (i == 0) {
